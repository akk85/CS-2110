...
public class GameMain {
    ...
    private static void createAndShowGUI() {
        ...
        // score.
        game.addPropertyChangeListener("GameScore", evt -> scoreLabel.setText("Score: " + game.getScore()));
        ...
        sizeSlider.addChangeListener((ChangeEvent e) -> {
            ...
            game.setTargetRadius(newSize);
        });

        // target duration to the slider's current value.
        speedSlider.addChangeListener((ChangeEvent e) -> {
            int newDuration = speedSlider.getValue();
    ...
    private static void saveScore(JFrame frame, int score) {
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            try (PrintWriter out =
                    new PrintWriter(new BufferedWriter(new FileWriter(fileToSave, true)))) {
                out.println(score);
            } catch (IOException e) {
                String errorMessage = e.getClass().getSimpleName();
                JOptionPane.showMessageDialog(frame, errorMessage + e.getMessage(), errorMessage, JOptionPane.ERROR_MESSAGE);
            }
        }

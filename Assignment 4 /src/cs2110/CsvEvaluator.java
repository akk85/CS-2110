...
public class CsvEvaluator {
     ...
     */
    public static void evaluateCsv(CSVParser parser, CSVPrinter printer)
            throws IOException {
        ...
        for (CSVRecord record : parser) {
            ...
            int colNumber = 1;
            // For each cell in the row, check if the cell starts with an '=' sign (indicating a formula).
            for (String cell : record) {
                if (cell.startsWith("=")) {
                    try {
                        String rpnExpression = translateFormulaToRpn(cell.substring(1), vars);
                        Expression result = RpnParser.parse(rpnExpression, defs);
                        double evalResult = result.eval(vars);
                        printer.print(evalResult);
                        vars.set(colToLetters(colNumber) + rowNumber, evalResult);
                    } catch (Exception e) {
                        printer.print("#N/A");
                    }
                } else {
                    ...
                    try {
                        double cellValue = Double.parseDouble(cell);
                        vars.set(colToLetters(colNumber) + rowNumber, cellValue);
                        printer.print(cell);
    ...
    private static String translateFormulaToRpn(String formula, VarTable vars) throws Exception {
        ...
        for (String token : tokens) {
            if (token.matches("^[A-Z]+[0-9]+$")) {
                ...
                translated.append(value).append(" ");
            } else {
                translated.append(token).append(" ");
            }
        }
    ...
    public static String colToLetters(int n) {
        ...
        if (n < 26) {
            ...
            int quotient = n / 26;  // Find the quotient
            int remainder = n % 26; // Find the remainder

            return colToLetters(quotient) + (char) ('A' + remainder);
        }
    }
